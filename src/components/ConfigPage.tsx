  ...
  const handleImportJSON = () => {
    try {
      ...
      const newServicos = data.servicos?.map((s: any) => ({
        ...
        status_atual: s.status_atual || 'nao_iniciado',
      ...
      const newNotas = data.notas?.map((n: any) => ({
        ...
        texto: n.texto,
        data_nota: n.data_nota || new Date().toISOString()
      })) || [];

      const newEquipes = data.equipes?.map((f: any) => ({
      ...
      toast(`Projeto inicializado: ${newServicos.length} serviços, ${newPendencias.length} pendências, ${newNotas.length} notas, ${newEquipes.length} equipes. Revise os dados e clique em Sync para salvar no banco.`, 'success');
      setJsonText('');
    } catch (e: any) {
